...
function testCtAddParsing() {
  ...
  const flatResponse = { status: 'added', 'report-id': 2389255 }

  // Our code: res.response?.['report-id'] ?? res['report-id']
  const reportIdFlat = flatResponse.response?.['report-id'] ?? (flatResponse as Record<string, unknown>)['report-id']
  ...
  const wrappedResponse = { response: { status: 'added', 'report-id': 682 } }
  const reportIdWrapped = wrappedResponse.response?.['report-id'] ?? (wrappedResponse as Record<string, unknown>)['report-id']
  ...
  const errorResponse = { success: false, errors: { location_id: 'Multiple reports for selected location.' } }
  const reportIdError = (errorResponse as Record<string, unknown>).response?.['report-id'] ?? (errorResponse as Record<string, unknown>)['report-id']
...
function testCtGetAllParsing() {
  ...
  const noResponse = {}
  const noResults = (noResponse as Record<string, unknown>).response?.results
  assert(!Array.isArray(noResults), 'missing response: not an array (returns null)')
}
...
function testCtGetParsing() {
  ...
  const report = docResponse.report || (docResponse as Record<string, unknown>).response
  assert(!!report, 'found report')
  assert((report as Record<string, unknown>).report_id === '255565', 'report_id is correct')
  assert((report as Record<string, unknown>).status === 'completed', 'status is correct')
  ...
  const altReport = (altResponse as Record<string, unknown>).report || altResponse.response
  assert(!!altReport, 'alt shape: found via response fallback')
  assert((altReport as Record<string, unknown>).report_id === '100', 'alt shape: report_id correct')
}
...
function testCtRunParsing() {
  ...
  const docResponse = { response: { status: 'running' } }

  // Our code: res.response?.status ?? res.status
  const status1 = docResponse.response?.status ?? (docResponse as Record<string, unknown>).status
  assert(status1 === 'running', 'wrapped: status is "running"')

  // What if flat?
  const flatResponse = { status: 'running' }
  const status2 = (flatResponse as Record<string, unknown>).response?.status ?? flatResponse.status
  assert(status2 === 'running', 'flat: status is "running"')

  // Error: report already running
  const errorResponse = { response: { status: 'already_running' } }
  const status3 = errorResponse.response?.status ?? (errorResponse as Record<string, unknown>).status
...
function testFindExistingCTReportLogic() {
  ...
  const res1 = { response: { results: [{ report_id: '278', location_id: '5' }, { report_id: '300', location_id: '5' }] } }
  ...
  const res3 = {} as Record<string, unknown>
  const results3 = (res3 as Record<string, unknown>).response?.results
  const found3 = (Array.isArray(results3) && (results3 as unknown[]).length > 0) ? String((results3 as Array<Record<string, unknown>>)[0].report_id) : null
...
function testCreateCTReportLogic() {
  ...
  const res1 = { status: 'added', 'report-id': 2389255 } as Record<string, unknown>
  const id1 = (res1 as Record<string, unknown>).response?.['report-id'] ?? res1['report-id']
  ...
  const res2 = { response: { status: 'added', 'report-id': 682 } }
  const id2 = res2.response?.['report-id'] ?? (res2 as Record<string, unknown>)['report-id']
  assert(id2 === 682, 'wrapped doc response: extracts report-id')

  // Case 3: Error response (no report-id)
  const res3 = { success: false, errors: { location_id: 'Multiple reports for selected location.' } } as Record<string, unknown>
  const id3 = (res3 as Record<string, unknown>).response?.['report-id'] ?? res3['report-id']
...
function testFullPipeline() {
  ...
  const locationsRes = { total_count: 2, items: [{ location_id: 123, business_name: 'Test Biz', country: 'USA' }] }
  ...
  const getAllRes = { response: { results: [{ report_id: '500', location_id: blLocId }] } }
  ...
  const getRes = { success: true, report: { report_id: '500', report_name: 'Test', status: 'completed' } }
  ...
  const addRes = { status: 'added', 'report-id': 600 } as Record<string, unknown>
  ...
  const runRes = { response: { status: 'running' } }
