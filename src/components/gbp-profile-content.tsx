...
import { GBPEditToggle } from './gbp-edit-toggle'
import { PerformanceChart } from './performance-chart'
import { ProfileAuditCard } from './profile-audit-card'
import { GoogleUpdatesBanner } from './google-updates-banner'
import { GBPMediaManager } from './gbp-media-manager'
import { GBPPostsSection } from './gbp-posts-section'
...
interface Props {
  ...
  reviewSource: { total_review_count: number; average_rating: number | null; last_synced_at: string | null } | null
  isAdmin: boolean
  locationId: string
  locationName: string
  basePath: string
...
export function GBPProfileContent({ profile, mediaItems, postItems, queuedPosts, reviewSource, isAdmin, locationId, locationName, basePath, orgSlug }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 text-xs">
        ...
        {isAdmin && !editing && (
          <GBPEditToggle onEdit={() => setEditing(true)} />
        )}
        {isAdmin && editing && (
          <button
      ...
      {isAdmin && gbp.has_google_updated && (
        <GoogleUpdatesBanner locationId={locationId} />
      )}

      {/* Optimization Score */}
      <ProfileAuditCard locationId={locationId} isAgencyAdmin={isAdmin} />
      ...
      {/* Performance Metrics */}
      <PerformanceChart locationId={locationId} />

      {/* CONDITIONAL: Edit mode OR Read-only mode */}
      {editing ? (
        <GBPProfileEditor
          profile={gbp}
      ...
      {/* Photos */}
      <GBPMediaManager mediaItems={mediaItems} locationId={locationId} isAdmin={isAdmin} />

      {/* Posts */}
      <GBPPostsSection posts={postItems} queuedPosts={queuedPosts} locationId={locationId} isAdmin={isAdmin} />
    </div>
  )
}
